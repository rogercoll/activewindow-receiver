...
type metricSystemGuiWindowTime struct {
	data     pmetric.Metric // data buffer for generated metric.
	config   MetricConfig   // metric config provided by user.
	capacity int            // max observed number of data points added to the metric.
}

// init fills system.gui.window.time metric with initial data.
func (m *metricSystemGuiWindowTime) init() {
	m.data.SetName("system.gui.window.time")
	m.data.SetDescription("Total seconds each window name has spent on each window id.")
	m.data.SetUnit("s")
	m.data.SetEmptySum()
...
func (m *metricSystemGuiWindowTime) updateCapacity() {
...
func (m *metricSystemGuiWindowTime) emit(metrics pmetric.MetricSlice) {
...
func newMetricSystemGuiWindowTime(cfg MetricConfig) metricSystemGuiWindowTime {
	m := metricSystemGuiWindowTime{config: cfg}
	if cfg.Enabled {
...
type MetricsBuilder struct {
	...
	metricSystemGuiWindowTime metricSystemGuiWindowTime
}
...
func NewMetricsBuilder(mbc MetricsBuilderConfig, settings receiver.Settings, options ...MetricBuilderOption) *MetricsBuilder {
	mb := &MetricsBuilder{
		...
		metricSystemGuiWindowTime: newMetricSystemGuiWindowTime(mbc.Metrics.SystemGuiWindowTime),
	}
...
func (mb *MetricsBuilder) EmitForResource(options ...ResourceMetricsOption) {
	...
	ils.Scope().SetName("github.com/rogercoll/activewindowreceiver")
	ils.Scope().SetVersion(mb.buildInfo.Version)
	ils.Metrics().EnsureCapacity(mb.metricsCapacity)
	mb.metricSystemGuiWindowTime.emit(ils.Metrics())
